...
Tidiers' obsession with keeping Tidy analysis self-contained. Objection
...
As noted, the *Educator's Perspective* article sets up a straw man, code
based on base-R's **aggregate**. The deem the code unacceptable for
these reasons:
...
* Base-R versions, using '$' etc., are typically simpler and more
  ...
  *Educator's Perspective'* authors conceded that the **tapply**
...
* The vast majority of statistical packages on CRAN make heavy use of
  those base-R constructs proscribed by the Tidiers. Indeed, many CRAN
  functions also violate the article's Objection 4. 

:::

The article contends that use of '$' is error-prone. But one could make
similar arguments for, say, **dplyr::mutate** being error-prone. Indeed,
