...
::: {.callout-important title='Synopsis'}
...
* It is not about teaching R to computer science students, nor is it
...
* The Tidy *functional programming* philosophy replaces straightforward,
  ...
  advocates concede that it is in various senses often more difficult to
  write Tidy code than base-R.  Even Hadley says, for instance, "it may
...
* Tidy has a function for everything under the sun, leading to cognitive
  overload, when many of these operations can be done simply in base-R.
  This is exacerbated by the Tidy philosophy that Tidy be
...
* Those knowing only Tidy are locked out from effectively using the vast
  ...
  the '$' operator, vectors, subscripts and so on.
...
* I taught the lowest level, ESL 50, adult students who barely knew the
  alphabet. The overriding theme I found is to start simply.  In
...
In other words, Tidy uses unnecessarily complicated machinery, an obvious
...
* I do NOT advocate a slow, boring approach in which, for instance, one
  starts with "R data can be real, character or logical..."  
...
I like and admire the Posit people, including the tidyverse originator,
Hadley Wickham, and have always supported them, both privately and
...
*Advanced R* (2nd. ed.).  I am writing this essay using Posit's 
**Quarto** package, a wonderful development for the R community.
...
of prominent Tidy advocates.  I did suggest a mixed approach to Posit
...
Posit should not be telling people how to teach.  But of course, that
is exactly what Posit has been doing in promoting Tidy.
